...
class apw_authors extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		// outputs the content of the widget
		 ...
		 $html = '<section class="add-nav widget widget_authors" id="apw-authors"><h2 class="widget-title">'.$title.'</h2>';
			...
			foreach($users as $results){
			 ...
			 if(!empty($result)){
				 ...
				 $name = (trim($name)==''?ucwords($user_info->display_name):$name);
			 	 $html .= '<div class="apw-closed"><a class="apw-parent">'.$name.' <span>('.count($result).')</span></a>';
				 $html .= '<ul>';
 	...
 	public function form( $instance ) {
        ...
        <p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Display excerpt?' ); ?></label>
